...
`,
	...
	const settings = {
		...
		"workbench.view.extension.Realtek.ameba-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.Realtek.ameba-ActivityBar",
			when: "viewContainer.workbench.view.extension.Realtek.ameba-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
	...
	const args = [
		...
		"--command",
		"workbench.view.extension.Realtek.ameba-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.Realtek.ameba-ActivityBar');
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						const extension = vscode.extensions.getExtension('Realtek.ameba');
						if (!extension) {
						...
						await vscode.commands.executeCommand('workbench.view.extension.Realtek.ameba-ActivityBar');
